...
const init = (container_selector) => {
    ...
    const $addButton = document.querySelector('#goal-answers-add');
    ...
    const addAnswer = () => {
        ...
        insertTemplate('goal_answer', template_data, container_selector);
        count++;
        last_id++;
    };
    ...
    bindLiveEvent('click', '.goal-answers-remove', (event, target) => {
        event.preventDefault();
        removeAnswer(target);
