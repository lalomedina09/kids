<section class="about-activities-section section-padding pt-0 pb-0 ct-action">
    <!--<div class="pencil-shape">
        <img src="img/about/pencil.png" alt="shape-img">
    </div>
    <div class="zebra-shape float-bob-y">
        <img src="img/about/zebra.png" alt="shape-img">
    </div>-->

    <div class="container">
        <div class="about-activities-wrapper">
            <div class="row g-4 pt-5">
                <div class="col-lg-6">
                    <div class="activities-content">
                        <div class="section-title">
                            <h2 class="wow fadeInUp text-purple mt-5" data-wow-delay=".3s">
                                Haz de tu escuela, un modelo
                                de educación financiera
                            </h2>
                        </div>
                        <p class="my-4 mt-md-0 wow fadeInUp" data-wow-delay=".5s">
                            Recibe una actividad de muestra adaptada a un grado específico.
                            <br><b>Gratis. Sin compromiso</b>
                            <br>
                            Haz clic y te enviamos un material para probar en tu escuela esta semana
                        </p>
                        <a href="news-details.html" class="theme-btn">
                            Solicitar demo
                        </a>
                    </div>
                </div>
                <div class="col-lg-6 wow fadeInUp" data-wow-delay=".4s">
                    <div class="activities-img-items">
                        <div class="activities-image">
                            <img src="img/ilustration/ilustrations-and-girl.png" alt="img"><!--510x485-->
                        </div>
                        <div class="radius-shape">
                            <img src="img/about/radius-shape-1.png" alt="img">
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
