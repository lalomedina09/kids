<div class="row">
    <div class="col-md-12">
        <h5 class="text-center" style="color:#fc7c7c">
            ¿Estas seguro de eliminar la categoría "{{ $categoryUser->name }}"?
        </h5>
    </div>
</div>
<div class="row">
    ...
    <div class="col-md-10">
        <div class="mt-2">
            ...
            <input type="hidden" id="formDeleteMove_name" value="{{ $categoryUser->name }}">
            <input type="hidden" id="formDeleteMove_id" value="{{ $categoryUser->move->id }}">
            <label class="containerCheck">
                <span class="small">
                    Si deseas quitar la categoría "{{ $categoryUser->name }}" existente en los próximos meses deja el check activado
                </span>
    ...
    <div class="col-md-11 text-center">
        <button class="button-confirm-delete"
            onclick="deleteCategoryBudget('{{$section}}', '{{ $divCategory }}', '{{ $divAmountEstimate }}', '{{ $divAmountReal }}', {{ $categoryUser->id }});">
            Eliminar categoría
        </button>
