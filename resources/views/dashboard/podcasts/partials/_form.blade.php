...
@push('scripts')
    ...
    <script type="text/javascript" src="{{ mix('js/dashboard/podcasts/create.js') }}"></script>
@endpush

@if (isset($update) && $update)
    {!! Form::model($podcast, ['route' => ['dashboard.podcasts.update', $podcast->id], 'method' => 'post', 'files' => true, 'id' => 'form-podcasts']) !!}
        @method('patch')
@else
    {!! Form::open(['route' => 'dashboard.podcasts.store', 'method' => 'POST', 'files' => true, 'id' => 'form-podcasts']) !!}
@endif

    <div class="row">
        <div class="col-md-8">
            <div class="form-group">
                <label for="title" class="form-label">Título del podcast:</label>
                {!! Form::text('title', null, ['class' => 'form-control input-lg', 'placeholder' => 'Título del Artículo'] ) !!}
            </div>

            <div class="form-group">
                <label for="body" class="form-label">Contenido del podcast:</label>
                {!! Form::textarea('body', null, ['class' => 'form-control wysiwyg', 'rows' => 40] ) !!}
            </div>

                <div class="form-group">
                <label for="excerpt" class="form-label">Extracto del podcast:</label>
        ...
        <div class="col-md-4">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="image-container">
                        @if( isset($podcast) )
                        {!! $podcast->present()->audio !!}
                        @endif
                    </div>
                    <div class="custom-file mt-4">
                        <input type="file" class="custom-file-input" name="audio_file" id="audio_file" accept="audio/*">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="image-container">
                        @if( isset($podcast) )
                            {!! $podcast->present()->video !!}
                        @endif
                    </div>
                    <div class="form-group">
                        {!! Form::text('url', null, ['class' => 'form-control', 'placeholder' => 'Url del Video Podcast'] ) !!}
                    </div>
                </div>
            </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="image-container">
                        @if( isset($podcast) AND $podcast->hasMedia('featured_image') )
                            <img src="{{ asset($podcast->present()->featured_image) }}" class="img-fluid" style="border: 1px solid #ddd;">
                        @endif
                    </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    @can('blog.podcasts.publish')
                        <div class="form-group">
                            <select name="author_id"
                                id="author" class="form-control">
                                @foreach ($authors as $id => $fullname)
                                    <option value="{{ $id }}"
                                        {{ ($podcast->author_id == $id) ? 'selected="selected"' : '' }}>
                                        {{ $fullname }}
                                    </option>
                                @endforeach
                            </select>
        ...
        <div class="row">
            <div class="col-xl-8 col-lg-8 col-md-8 col-sm-8 col-12">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        @if( isset($podcast) and isset($podcast->published_at) )
                            <p>
                                <span class="font-weight-bold">Publicado:</span> {{ $podcast->present()->published_at }}
                                <a href="{{ route('dashboard.podcasts.unpublish', [$podcast->id]) }}" class="btn btn-sm btn-outline-warning d-inline"
                                    data-method="delete"
                        ...
                        <form action="{{ route('dashboard.podcasts.publish', [$podcast->id]) }}" method="post">
                            @csrf
                            ...
                            <div class="input-group mb-3">
                                ...
                                <input type="datetime" name="published_at" id="podcast-published_at" class="form-control datetimepicker-input" data-target="#podcast-published_at" data-toggle="datetimepicker">
                            </div>
