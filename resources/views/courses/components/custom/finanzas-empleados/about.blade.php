<section>
    <!--<div class="container">-->
        <div class="row">
            <div class="col-md-6 col-12 image-background mt-3" style="background-image: url(&#39;{{ asset('images/courses/finanzas-personales-empleados/sobre-curso.png') }}&#39;);">

            </div>

            <div class="col-md-6 col-12 image-background mt-3" style="background-image: url(&#39;{{ asset('images/courses/finanzas-empleados/background.png') }}&#39;);">
                <div class="row mt-5">
                    <div class="col-md-10 mb-5">
                        <h2 class="mb-3 pt-3 text-white font-weight-normal title-underline title-underline-b title-underline-left-black banner-title-course">
                            <span class="font-weight-bold">Sobre este curso</span>
                        </h2>
                        <br>
                        <p class="font-weight-normal text-white">
                            Este curso ayudará a tu equipo a tener
                            <b> una relación más sana con sus finanzas
                            personales</b>, ayudándoles a conocer y
                            entender los conceptos básicos de
                            administración, ahorro y creación de
                            patrimonio de una forma sencilla y
                            divertida, pero profunda y reveladora.
                        </p>

                        <p class="font-weight-normal text-white">
                            Explicamos los conceptos financieros con
                            <b>un lenguaje sencillo y fácil de entender.</b>
                        </p>

                        <p class="font-weight-normal text-black">
                            <b>Formatos disponibles:</b>
                        </p>
                        <img src="{{ asset('images/courses/finanzas-personales-empleados/virtual.png') }}" alt="Curso Virtual" width="8%" class="img-movil-center">
                        <span class="font-weight-normal text-white">Virtual</span>
                        <br><br>
                        <img src="{{ asset('images/courses/finanzas-personales-empleados/presencial.png') }}" alt="Curso Presencial" width="5%" class="img-movil-center"
                            style="margin-left: 7px; margin-right: 6px;">
                        <span class="font-weight-normal text-white">Presencial</span>
                    </div>
                </div>
            </div>

        </div>
    <!--</div>-->

</section>
