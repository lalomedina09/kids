                    ...
                    <div class="col-md-6" style="position: relative;">
                        ...
                        <div class="d-flex justify-content-end gap-2 reaction-container"
                            ...
                            <div id="reaction-menu" class="reaction-menu d-none"
                                ...
                                <button class="btn btn-sm btn-dark font-akshar btn-share w-100 text-left"
                                    title="Compartir" style="border-radius: 0;">
                                    <i class="fas fa-share-alt"></i> Compartir
                                </button>
                                <button class="btn btn-sm btn-dark font-akshar btn-like w-100 text-left"
                                    data-article-id="{{ $article->id }}" title="Me gusta"
                                ...
                                <button class="btn btn-sm btn-dark font-akshar btn-bookmark w-100 text-left"
                                    data-article-id="{{ $article->id }}" title="Guardar"
                                    @guest data-toggle="modal"
                                    data-target="#modalLogin" @endguest>
                            ...
                            <div class="d-none d-md-flex gap-2">
                                ...
                                <button class="btn btn-sm btn-dark font-akshar btn-like"
                                    data-article-id="{{ $article->id }}" title="Me gusta" style="margin-right: 10px;"
                                    @guest data-toggle="modal" data-target="#modalLogin" @endguest>
                                    <i class="fas fa-heart"></i>
    ...
    $(document).ready(function() {
        ...
        $('.btn-bookmark').on('click', function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    const buttons = $(`.btn-bookmark[data-article-id="${articleId}"]`);
                    if (data.is_bookmarked) {
                        buttons.html('<i class="fas fa-bookmark"></i> Guardado');
                        buttons.removeClass('btn-dark').addClass('btn-success');
                    } else {
                        buttons.html('<i class="fas fa-bookmark"></i> Guardar');
                        buttons.removeClass('btn-success').addClass('btn-dark');
                    }
                    Swal.fire({
                        icon: 'success',
                        title: data.message,
        ...
        $('.btn-like').on('click', function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    const buttons = $(`.btn-like[data-article-id="${articleId}"]`);
                    if (data.is_liked) {
                        buttons.html('<i class="fas fa-heart"></i> Te gusta');
                        buttons.removeClass('btn-dark').addClass('btn-success');
                    } else {
                        buttons.html('<i class="fas fa-heart"></i> Me gusta');
                        buttons.removeClass('btn-success').addClass('btn-dark');
                    }
                    Swal.fire({
                        icon: 'success',
